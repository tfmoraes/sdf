...
def voxelize(
    ...
    Returns:
        ...
        spacing: distance between samples (X, Y, Z)
        offset: position of first value in volume (X, Y, Z)
    """
    start = time.time()
    if bounds is None:
        bounds = _estimate_bounds(sdf)
    ...
    coordinates = _cartesian_product(X, Y, Z)

    volume = sdf(coordinates).reshape(X.shape[0], Y.shape[0], Z.shape[0])
    if verbose:
        seconds = time.time() - start
        print(f"{volume.size:_} voxels in {seconds} seconds")

    return (volume, (dx, dy, dz), offset)
